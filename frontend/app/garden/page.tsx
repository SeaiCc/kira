...
export default function GardenPage() {
  ...
  const [trendData, setTrendData] = useState<Array<{ date: string; 文章: number }>>([]);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      const map = new Map<string, { 文章: number }>();
      posts.forEach((p) => {
        const d = (p.published_at ?? p.created_at).slice(0, 10);
        const entry = map.get(d) ?? { 文章: 0 };
        entry.文章++;
        map.set(d, entry);
      });
    ...
    Promise.all([
      ...
      setCategoryData([
        ...
        { name: "照片", value: photoCount, color: colors[2] },
      ]);
    }).catch(() => {});
  }, []);
              ...
              <PieChart>
                ...
                <Tooltip
                  formatter={(value, name) => {
                    const units: Record<string, string> = { 文章: "篇", 照片: "张" };
